...
export const GameHeader = ({
  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      const {
        data
      } = await supabase.from('characters').select('current_hp, max_hp, experience_points, level').eq('id', characterId).single();
      if (data) {
    ...
    const channel = supabase.channel(`character-stats-${characterId}`).on('postgres_changes', {
      event: 'UPDATE',
      schema: 'public',
      table: 'characters',
  ...
  return <header className="relative border-b border-border/50 backdrop-blur-epic bg-card/30">
      <div className="container mx-auto px-3 md:px-6 py-3 md:py-6">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 md:gap-3 flex-shrink-0">
            {/* Character Stats */}
            {stats && <div className="hidden lg:flex flex-col gap-1.5 mr-4 min-w-[200px]">
                {/* HP Bar */}
                <div className="flex items-center gap-2">
                  <Heart className="h-3.5 w-3.5 text-destructive" />
                  <div className="flex-1">
                    <Progress value={hpPercentage} className="h-2 bg-muted" indicatorClassName={hpPercentage > 50 ? "bg-green-500" : hpPercentage > 25 ? "bg-yellow-500" : "bg-destructive"} />
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    <Progress value={xpPercentage} className="h-2 bg-muted" indicatorClassName="bg-primary" />
                  </div>
                  <span className="text-xs font-medium min-w-[45px] text-right">
                    Nv {stats.level}
                  </span>
            ...
            {/* Players Panel */}
            {players.length > 0 && gmId && <Sheet>
                <SheetTrigger asChild>
                  <Button variant="outline" size="sm" className="gap-1 md:gap-2 h-7 md:h-9 px-2 md:px-3">
                    <Users className="h-3 w-3 md:h-4 md:w-4" />
                    <span className="hidden sm:inline text-xs md:text-sm">{players.length}</span>
                  </Button>
                </SheetTrigger>
                <SheetContent side="right" className="w-[300px] sm:w-[400px] overflow-y-auto">
                  <PlayersPanel players={players} currentUserId={currentUserId} gmId={gmId} />
                </SheetContent>
              </Sheet>}
            
            {onBackToLobby && <Button variant="outline" size="sm" onClick={onBackToLobby} className="gap-1 md:gap-2 h-7 md:h-9 px-2 md:px-3">
                <ArrowLeft className="h-3 w-3 md:h-4 md:w-4" />
                <span className="hidden sm:inline text-xs md:text-sm">Lobby</span>
              </Button>}
            
            {onBackToCharacterSelect && !roomCode && <Button variant="outline" size="sm" onClick={onBackToCharacterSelect} className="gap-1 md:gap-2 h-7 md:h-9 px-2 md:px-3">
                <Users className="h-3 w-3 md:h-4 md:w-4" />
